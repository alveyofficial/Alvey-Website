       ...
       <>
        ...
        {/* Logo */}
                    <Link to="/" className="flex items-center gap-2.5 group">
                        <img src='/logo.webp' className="w-12 h-12 transition-transform duration-300 group-hover:scale-110"/>
                        <div className="leading-none">
                            <h1 className="font-black text-xl tracking-tight text-[#164E5E] dark:text-cyan-300">ALVEY</h1>

                            <p className="text-[10px] tracking-[0.25em] uppercase text-[#7D868C]">Study Better</p>
                        </div>
                    </Link>
          ...
          <div className="px-4 pt-2 pb-6 space-y-1.5">
            {navLinks.map((link) => (
               <NavLink
                ...
                className={({ isActive }) => isActive ? "bg-[#164E5E]/10 text-[#164E5E] dark:bg-[#3D7F8F]/20 dark:text-[#6FD4D8] font-semibold" : "block text-base font-medium text-muted-foreground hover:text-foreground px-4 py-2.5 rounded-xl transition-colors"
                }>
                {link.label}
              </NavLink>
           ))}
