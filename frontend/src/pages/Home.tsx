...
export const meta: MetaFunction = () => {
  ...
  return [
    ...
    ...scripts.map((script) => ({
      ...
      innerHTML: typeof script === "string" ? script : JSON.stringify(script),
    })),
  ];
};
...
export default function Home(){
    ...
    (async () => {
      try {
        const docs = await api.getSubjectCategories();
        if (docs.length > 0) {
            const categories: Level[] = docs.map((doc :any)=> ({
    ...
    useEffect(() => {
        api.getHomepageStats().then(setStats);
    }, []);
        ...
        <div className="max-w-7xl mx-auto px-8 py-16">
          ...
          <div className="flex flex-col md:flex-row justify-between items-center gap-4 text-gray-700 dark:text-gray-300 text-sm">
            ...
            <p className="font-medium">
              Built with ❤️ by the Alvey Team
            </p>
